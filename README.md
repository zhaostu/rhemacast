# rhemacast — near-realtime live audio broadcast (mic -> Pi -> phones)

Audio-only live broadcast. Speak into a mic on a Linux box; 5-20 phones listen with <500ms latency, fully offline.
...
- `server/web/admin.html` — broadcast dashboard (embedded): `http://<pi-ip>:8080/admin.html` (LIVE dot, mic meter, listeners, start/stop, logs).
...
Both phone apps have a `Broadcast controls` link opening the admin page for the entered IP.

...
/**
 * Audio-only WebRTC receiver using WHEP against MediaMTX.
 *
...
class WebRTCClient(
    ...
    suspend fun connect(hostIp: String) {
        ...
        try {
            ...
            // Route to music stream; actual decoding/rendering is done by WebRTC ADM.
    ...
    private suspend fun waitForIceGathering(pc: PeerConnection, timeoutMs: Long) {
        ...
        // MediaMTX WHEP works with a plain non-trickle offer, so a bounded wait
    ...
    private suspend fun postWhep(url: String, offerSdp: String): String {
        ...
        res.use {
            ...
            return (it.body?.string() ?: "").trim().ifEmpty {
                throw java.io.IOException("Empty WHEP answer")
            }
        }
    }
